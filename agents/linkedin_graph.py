...
from agents.prompts import RESEARCH_PROMPT, LINKEDIN_WRITER_PROMPT, LINKEDIN_CRITIC_PROMPT
...
def writer_node(state: EmailState) -> dict:
    ...
    if critic and not critic.get("pass", True):
        ...
        revision_count += 1
        log.append(f"[LinkedIn Writer] Revising (revision {revision_count})")
    else:
        log.append("[LinkedIn Writer] Writing post")

    resp = _get_llm(0).invoke([
        SystemMessage(content=LINKEDIN_WRITER_PROMPT),
        HumanMessage(content=user_msg)
    ])
    try:
        copy = json.loads(_strip(resp.content))
    except:
        copy = {"post_text": resp.content, "hook": "", "hashtags": []}
...
def critic_node(state: EmailState) -> dict:
    log = state.get("pipeline_log", [])
    resp = _get_llm(0).invoke([
        SystemMessage(content=LINKEDIN_CRITIC_PROMPT),
        HumanMessage(content=f"Scenario: {state['scenario']}\n\nPost:\n{json.dumps(state.get('email_copy',{}), indent=2)}")
    ])
    try:
        result = json.loads(_strip(resp.content))
    except:
        result = {"hook_score":0,"brand_voice_score":0,"engagement_score":0,
                  "accuracy_score":0,"overall_score":0,"pass":False,
                  "feedback":"Parse error","flagged_claims":[]}
    log.append(f"[LinkedIn Critic] Score: {result.get('overall_score')} | Pass: {result.get('pass')}")
...
def finalize_node(state: EmailState) -> dict:
    ...
    if not critic.get("pass", True):
        needs_review.append("Post did not pass quality review — needs human edit before publishing.")
